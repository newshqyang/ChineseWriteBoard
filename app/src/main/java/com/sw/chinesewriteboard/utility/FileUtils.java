...
public class FileUtils {
    ...
    public static boolean searchTtfFiles(String path, List<MyTypeface> foundList) {
        File fileDir = new File(path);
        File[] files = fileDir.listFiles();
        if (files != null) {
            for (File f : files) {
                if (f.getName().toLowerCase().endsWith(".ttf") && !f.getName().startsWith(".")) {
                    String fontName = f.getName().toLowerCase().replace(".ttf", "");
                    if (fontName.trim().length() > 0) {
                        foundList.add(new MyTypeface(fontName, Typeface.createFromFile(f.getAbsolutePath())));
                    }
                }
                // 如果是文件夹 继续读取
                if (f.isDirectory()) {
                    searchTtfFiles(f.getAbsolutePath(), foundList);
                }
            }
        }
        return true;
